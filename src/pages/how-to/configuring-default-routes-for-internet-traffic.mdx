...
Administrators configure exit nodes from the dashboard, and can optionally mark the default route (0.0.0.0/0) as selected by default. Clients will then auto-apply the selected exit node unless the route is configured with Skip Auto Apply or the user has made a local choice on the device.

- **Skip Auto Apply**: when enabled on an exit node route, clients will not auto-apply that exit node. Users can still manually enable it from the client if they choose to use that exit node.
...
<Note>
  Exit node auto-apply and client override behavior requires NetBird client v0.55.0 or later.
</Note>
...
If you want exit nodes to be available without being automatically enabled on clients, enable the **Skip Auto Apply** option. When Skip Auto Apply is on, clients will not auto-apply the exit node, but users can manually enable it from the client.

<p>
    <img src="/docs-static/img/how-to-guides/exit-node-skip-auto-apply.png" alt="exit-node-skip-auto-apply" className="imagewrapper-big"/>
</p>
