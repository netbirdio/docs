...
NetBird do not store Network Flow Logs indefinitely; instead, we follow a retention policy to
...
To enhance monitoring and centralized analysis, we support shipping flow logs to external logging solutions.
Currently, DataDog is the only supported destination for log shipping.
This integration allows you to seamlessly forward Network Flow Logs from NetBird to DataDog,
...
Flow logs in this scenario can highlight if any unexpected access is happening.
...
Flow logs will capture the actual IP connections made, but it’s important to understand how domain-based rules reflect in the logs.
...
By default, the Network Flow Log records only the resolved IP address, not the original domain name.
This approach aligns with NetBird’s default behavior of not logging DNS queries to maintain privacy.
...
Network Flow Logs become a powerful tool to monitor and troubleshoot this inter-site traffic.
...
Here’s how the flow logs play out:
...
Essentially, the flow logs allow you to break the problem into segments (A -> tunnel, tunnel -> B, B -> host, etc.).

Even in normal operation, site-to-site flow logs give visibility into the volume and types of traffic flowing between locations.
...
Network Flow Logs in NetBird provide network administrators with valuable visibility into the traffic within their secure network, all while operating at a high level that avoids delving into packet contents.
In this documentation, we covered what flow logs are and how they function on NetBird clients:
