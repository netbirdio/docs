...
The connection proceeds to that IP over the tunnel. The flow log on the peer will show a connection to that IP address (since that’s what ultimately happens on the network layer).
