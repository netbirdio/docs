...
Similarly, NetBird’s own control traffic (management, keepalives, etc.) is not recorded in Traffic Events.
...
NetBird do not store Traffic Events indefinitely; instead, we follow a retention policy to
...
This integration allows you to seamlessly forward Traffic Events from NetBird to DataDog,
...
or any application data exchanged between two peers over the NetBird.
...
* **[CHECK]** On Routing Peer A’s log, you would likely see two related flows: one coming from Peer A
(source Peer A’s NB IP to Routing Peer A’s NB IP) and one going out to the server
(source Routing Peer A’s LAN IP to `192.168.0.201` on port 443). Routing Peer A essentially bridges the two networks,
so it sees an incoming encrypted flow and forwards the traffic internally.
NetBird’s flow logging on Routing Peer A can capture the incoming portion (the NetBird tunnel from Peer A)
...
you could check Routing Peer A’s logs to ensure it forwarded the traffic. If Routing Peer A’s log shows the flow from Peer A but
