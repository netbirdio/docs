...
For this guide I will be running a set of 4 Ubuntu Virtual Machines backed by `libvirt` split into 2 separate networks
as follows (I've stripped down unnecessary pieces of information):
```shell
root@vm-host ~# virsh net-dhcp-leases default
... IP address           Hostname  ...
... -------------------------------...
... 192.168.122.144/24   nb-01     ...
... 192.168.122.65/24    ubuntu-01 ...

root@vm-host ~# virsh net-dhcp-leases net2
... IP address      Hostname           ...
...------------------------------------...
... 10.0.0.189/24   nb-isolated-01     ...
... 10.0.0.215/24   ubuntu-isolated-01 ...
```

All VMs can be reached from the host `vm-host`:
```shell
kdn@my-laptop ~> ssh vm-host.lan ping -c1 10.0.0.189
PING 10.0.0.189 (10.0.0.189) 56(84) bytes of data.
64 bytes from 10.0.0.189: icmp_seq=1 ttl=64 time=0.154 ms

--- 10.0.0.189 ping statistics ---
1 packets transmitted, 1 received, 0% packet loss, time 0ms
rtt min/avg/max/mdev = 0.154/0.154/0.154/0.000 ms
kdn@my-laptop ~> ssh vm-host.lan ping -c1 192.168.122.251
...
```shell
kdn@my-laptop ~> ssh 10.0.0.189 -J vm-host.lan ping -c1 192.168.122.251
PING 192.168.122.251 (192.168.122.251) 56(84) bytes of data.
...
Devices `ubuntu-01` and `ubuntu-isolated-01` are **clientless** for purposes of this guide,
additionally `ubuntu-isolated-01` runs an internal CoreDNS on the IP address `10.0.0.10` responding with `OK`
to a HTTP health check at `http://10.0.0.10/health`.

The **Peers** look as follows:

| dns_label        | netbird_ip       | groups      |
|------------------|------------------|-------------|
| `nb-01`          | `100.83.73.97`   | `m:group-a` |
| `nb-isolated-01` | `100.83.136.209` | `m:group-r` |

We will be granting access between:
- `ubuntu-01` running at a `default` Site through a Routing Peer group `m:group-a`,
- `nb-isolated-01` running at `net1` Site through a Routing Peer group `m:group-r`,
...
```shell
kdn@my-laptop ~> ssh nb-01.lan "netbird networks ls"
Available Networks:
...
kdn@my-laptop ~> ssh nb-01.lan "ping -c1 10.0.0.10"
PING 10.0.0.10 (10.0.0.10) 56(84) bytes of data.
64 bytes from 10.0.0.10: icmp_seq=1 ttl=63 time=0.475 ms

--- 10.0.0.10 ping statistics ---
1 packets transmitted, 1 received, 0% packet loss, time 0ms
rtt min/avg/max/mdev = 0.475/0.475/0.475/0.000 ms

kdn@my-laptop ~> ssh nb-01.lan "curl 10.0.0.10/health; echo"
OK
```

#### Setting up the clientless device

Let's instruct a **clientless** `ubuntu-01` to consume the `net1` site
through `nb-01`'s local IP address `192.168.122.144`:
```shell
kdn@my-laptop ~> ssh ubuntu-01.lan "ip route | grep 192.168.100"
kdn@my-laptop ~ [1]> ssh ubuntu-01.lan "sudo ip route add 10.0.0.0/24 via 192.168.122.144"
kdn@my-laptop ~> ssh ubuntu-01.lan "ip route | grep 192.168.100"
10.0.0.0/24 via 192.168.122.144 dev enp7s0 
```
...
```shell
kdn@my-laptop ~> ssh ubuntu-01.lan "ping -c1 10.0.0.10"
PING 10.0.0.10 (10.0.0.10) 56(84) bytes of data.
...
kdn@my-laptop ~> ssh ubuntu-01.lan "ping -c1 10.0.0.10"
PING 10.0.0.10 (10.0.0.10) 56(84) bytes of data.
64 bytes from 10.0.0.10: icmp_seq=1 ttl=62 time=0.867 ms
...
Let's fetch `ubuntu-01` local Site IP, perform the same setup on `ubuntu-isolated-01` and test the
access back from the remote `net1` site:
```shell
kdn@my-laptop ~> ssh ubuntu-01.lan "ip a | grep 192.168.122"
    inet 192.168.122.65/24 metric 100 brd 192.168.122.255 scope global dynamic enp7s0
kdn@my-laptop ~> ssh 10.0.0.215 -J vm-host.lan "ip route | grep 192.168.122"
kdn@my-laptop ~ [1]>  ssh 10.0.0.215 -J vm-host.lan "sudo ip route add 192.168.122.0/24 via 10.0.0.189"
kdn@my-laptop ~> ssh 10.0.0.215 -J vm-host.lan "ip route | grep 192.168.122"
192.168.122.0/24 via 10.0.0.189 dev enp7s0
kdn@my-laptop ~> ssh 10.0.0.189 -J vm-host.lan "ping -c1 192.168.122.65"
...
kdn@my-laptop ~> ssh ubuntu-01.lan "ping -c1 10.0.0.10"
PING 10.0.0.10 (10.0.0.10) 56(84) bytes of data.
64 bytes from 10.0.0.10: icmp_seq=1 ttl=62 time=0.738 ms

--- 10.0.0.10 ping statistics ---
1 packets transmitted, 1 received, 0% packet loss, time 0ms
rtt min/avg/max/mdev = 0.738/0.738/0.738/0.000 ms
kdn@my-laptop ~> ssh ubuntu-01.lan "curl -sv -m 2 10.0.0.10/health; echo"
*   Trying 10.0.0.10:80...
...
kdn@my-laptop ~> ssh 10.0.0.215 -J vm-host.lan "ip route | grep 192.168.122"
192.168.122.0/24 via 10.0.0.189 dev enp7s0
kdn@my-laptop ~> ssh 10.0.0.215 -J vm-host.lan "ping -c1 192.168.122.65"
PING 192.168.122.65 (192.168.122.65) 56(84) bytes of data.
...
[Site-to-Site using Network Routes with Masquerading and Access Control](#site-to-site-using-network-routes-with-masquerading-and-access-control) 
configuration and verify that it's working. We will start by setting up a Network for each site, enable the minimal 
set of (already existing) Access Policies required to make the setup work and finally verify it is usable.
...
and enable the 2 required Access Policies:
...
<Note>
  Unlike **Network Routes**, you don't need policies granting access to the Routing Peers, but a reverse access policy
  is mandatory, otherwise the reverse route will not be advertised on the remote end and the answers will not be routed
  back.
</Note>
...
kdn@my-laptop ~> ssh ubuntu-01.lan "ping -c1 10.0.0.10"
PING 10.0.0.10 (10.0.0.10) 56(84) bytes of data.
64 bytes from 10.0.0.10: icmp_seq=1 ttl=62 time=0.783 ms
...
```shell
kdn@my-laptop ~> ssh ubuntu-01.lan "curl -m 2 10.0.0.10/health; echo"
curl: (28) Connection timed out after 2002 milliseconds
```

#### Asymmetric Network Resource policies

The reverse Access Policy does not need to match the protocol and access level of the forward policy,
already established connections will be routed back just fine as long as the reverse route itself is registered
on the remote end.
...
kdn@my-laptop ~> ssh ubuntu-01.lan "ping -c1 10.0.0.10"
PING 10.0.0.10 (10.0.0.10) 56(84) bytes of data.
64 bytes from 10.0.0.10: icmp_seq=1 ttl=62 time=0.836 ms
...
kdn@my-laptop ~ [1]> ssh 10.0.0.215 -J vm-host.lan "nc -w 2 192.168.122.65 22"
...
In short you should be able to simply switch off Masquerading on each Network Route from
...
- a pair of Access Policies granting Routing Peers access to each other,
- manually set up **clientless** devices with routes through the respective Routing Peers. 

The Network Routes (just like [above](#routes-noacl-site-to-site)):
...
Note you will need to turn off Masquerading in the Network Route's details dialog (it is not visible in the list view):
...
The Access Policies (just like [above](#unidirectional-routing-peer-policy)):
...
Having above in place we can confirm that the `ping` works:
...
kdn@my-laptop ~> ssh ubuntu-01.lan "ping -c1 10.0.0.10"
PING 10.0.0.10 (10.0.0.10) 56(84) bytes of data.
64 bytes from 10.0.0.10: icmp_seq=1 ttl=62 time=0.897 ms
...
```shell
kdn@my-laptop ~> ssh 10.0.0.10 -J vm-host.lan "sudo tcpdump -nvv -i any --immediate-mode -l icmp"
...
17:32:17.845428 enp7s0 In  IP (tos 0x0, ttl 62, id 56506, offset 0, flags [DF], proto ICMP (1), length 84)
    192.168.122.65 > 10.0.0.10: ICMP echo request, id 4480, seq 1, length 64
17:32:17.845468 enp7s0 Out IP (tos 0x0, ttl 64, id 51781, offset 0, flags [none], proto ICMP (1), length 84)
    10.0.0.10 > 192.168.122.65: ICMP echo reply, id 4480, seq 1, length 64
^C⏎
```
