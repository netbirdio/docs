...
Before starting the integration process, verify that you have the required permissions in Microsoft Entra ID (Azure AD). Specifically, you will need an Azure user account with at least one of these roles:
...
## Creating and Configuring a Microsoft Entra ID Application for NetBird Integration
...
Copy and securely store the generated `Aplication (client) ID` and `Directory (tenant) ID` as you will need them shortly.

## Configuring API Permissions for NetBird-Entra ID Integration
...
## Creating a Client Secret for Secure NetBird-Entra ID Authentication
...
## Entering Application ID and Directory ID in NetBird
...
![NetBird Identity Provider Syncronized](/docs-static/img/how-to-guides/microsoft-entra-id-sync/DH5hxFK.png)

To verify the syncronization, navigate to `Teams > Users` in the left menu.
