...
<Note>
    <strong>Note for Self-hosted:</strong> This feature also requires an upgraded NetBird management server. Make sure your server is updated to the <strong>v0.45.0</strong> version that supports lazy connections.
</Note>
...
- **Enabled:**  
  The client’s lazy connection manager is activated. Existing peer connections and ongoing connection attempts remain unchanged for **1 hour**. After that, any connection that couldn’t be established due to unavailable remote peers will transition to an **idle** state.

- **Disabled:**  
  The lazy connection manager is deactivated. The client will **immediately try to connect** to all remote peers.
