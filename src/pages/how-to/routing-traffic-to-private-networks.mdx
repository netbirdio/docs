...
For highly availability routes, you have two options:
...
To enable high-availability mode, click on `Add Peer` in the High Availability column in the Network Routes table and select a new peer in the `Routing Peer` field. Then select the `Distribution Groups` and click on `Add Route`.
...
In the following example (see column `Distribution Groups`), peers that belong to group `berlin-office` will use `aws-nb-europe-router-az-a` routing peer to access the `aws-eu-central-1-vpc` network. While peers that belong to group `london-office` will use `aws-nb-europe-router-az-b` routing peer.
