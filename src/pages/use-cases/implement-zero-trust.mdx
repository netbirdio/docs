...
- **[Audit events logging](/manage/activity)** for policy and configuration change history
...
- **Connection traffic events logging** for flow visibility (see [Section 7](#7-visibility-traffic-events-siem-and-control-center) and [Traffic Events](/manage/activity/traffic-events-logging))
- **Audit and traffic events streaming** to Datadog, S3, Firehose, generic HTTP for SIEM (Security Information and Event Management) integration (see [Section 7](#7-visibility-traffic-events-siem-and-control-center) and [Event Streaming](/manage/activity/event-streaming))
...
If you self-host, make sure Management (the control plane), Signal (peer coordination service), and Relay (fallback connection service) are on supported versions for these features.

### 0.2 Firewall rules for NetBird traffic

NetBird peers do not need inbound ports open. They initiate outbound connections to NetBird's Management, Signal, and Relay services over HTTPS (TCP port 443) and UDP/STUN/TURN protocols. If your network firewall restricts outbound traffic (egress filtering), you must allow these specific endpoints and ports.
...
- **Management service**: The control plane that manages peer authentication, policies, and configuration
...
# Controls management plane access - required for all NetBird operations
ALLOW OUT TCP 443 to api.netbird.io
# Signaling service (peer coordination) - required for peer discovery
ALLOW OUT TCP 443 to signal.netbird.io

# 2) NAT traversal support (recommended for reliable P2P):
# STUN servers help peers establish direct connections through NATs/firewalls
ALLOW OUT UDP 80,443,3478,5555 to stun.netbird.io

# 3) Fallback relay servers:
# Used when direct peer-to-peer connections fail
...
- **Routing peer**
    ...
    A NetBird peer that acts as a gateway, forwarding traffic between the NetBird overlay network and one or more internal subnets (private network ranges). It usually runs in a VPC (Virtual Private Cloud, like AWS VPC or Azure VNet), data center, or office LAN (Local Area Network). Think of it as a bridge between NetBird's virtual network and your physical/internal networks.
    
- **Overlay network and overlay IP**
    
    The overlay network is a virtual network that NetBird creates on top of your existing network infrastructure (like a "network on top of a network"). For each NetBird tenant, the overlay will be a random /16 CIDR block within the `100.64.0.0/10` range (a private IP range reserved for carrier-grade NAT). Each NetBird peer receives an overlay IP address from this /16 range, which is used for communication between peers. Internal networks never see WireGuard encryption keys or tunnels directly—they only see overlay IPs or the routing peer's IP address, depending on whether masquerading is enabled (see Section 6.2).
    
- **Network (NetBird object)**
    
    A NetBird configuration object that maps your internal networks (such as VPCs, LANs, or office networks) and organizes routing peers and resources. [Networks](/manage/networks) group routing peers that provide access to the same internal subnets. Resources within Networks (IP addresses, ranges, or DNS names) are used as destinations in access policies.
...
- **SIEM (Security Information and Event Management)**
    ...
    A security platform that collects, analyzes, and stores security events from various sources. Examples include Datadog, AWS Firehose, or S3-based pipelines. SIEMs ingest NetBird events (traffic flows, policy changes, authentication events) for visibility, alerting, and compliance reporting.
...
- **Scope**
    ...
    A bounded set of users, groups, peers and resources that belong together operationally, for example `prod-orders-app`, `prod-admin-portal` or `dev-tooling`.
...
Capture this as a simple matrix: users/groups/peers on one axis, resources on the other, with allowed protocols and ports in each cell.
...
> For the full traffic discovery workflow that you will reuse per scope, see [Section 7.2](#72-use-the-siem-for-discovery-and-verification), which covers flow discovery using event logs. Enable Traffic Events and streaming now so you can use that workflow as soon as peers are enrolled.
> 
...
To avoid early confusion, NetBird creates a Default policy that allows every peer to talk to every other peer on all protocols and ports in new environments. This is useful only as a temporary onboarding crutch.
...
<Note>
**Security warning:** Posture checks are not a security boundary against compromised endpoints. They validate configuration and policy compliance, not trustworthiness. An attacker with admin rights on a device can bypass or spoof them. Design your policies as if posture checks can be evaded.
</Note>
...
- **Geolocation or peer network range**
    - Restrict access to known countries or corporate CIDRs (IP address ranges, like `10.0.0.0/8` or `192.168.1.0/24`).
- **Process checks**
...
- Portless rules are effectively "all ports" and should be treated as wide open (not recommended for Zero Trust).
...
Take a simple production orders app with a three-tier architecture (web frontend, application backend, database):
...
- Each routing peer for a Network has a metric. The lower metric is preferred and will be the chosen routing peer for traffic.
- If multiple routing peers share the same metric, and the difference in latency is more than 10ms, traffic will switch to the faster routing peer.
...
- Use network/route selection to avoid accidental routing into the wrong VPC or site when CIDRs overlap. This can be done in the systray application, or command line.
...
NetBird's [Control Center](/manage/control-center) gives you a topological view of who can reach what, based on policies, groups and Networks. Access control policies can be edited directly from the control center.
...
<p>
    <img src="/docs-static/img/manage/control-center/control-center-groups-view.png" alt="Control Center groups view showing group access topology" className="imagewrapper-big"/>
</p>

- **Group view**
    - Select a group of users and see which resources they can reach and through which policies.
...
These dashboard settings enhance your Zero Trust security posture by adding additional layers of access control and authentication verification.

### 10.1 Settings - Authentication

Configure these authentication settings to strengthen peer and user verification:

Where to configure: **Settings → Authentication** in the NetBird dashboard.

- **Manual peer approval enabled**
    
    Require administrator approval before new peers can join the network. This prevents unauthorized devices from automatically connecting and gives you control over which devices are allowed access.
...
- **Peer session expiration set to the default of 24 hours or lower**
    
    Configure peer sessions to expire after 24 hours or less, requiring users to re-authenticate regularly. This limits the window of opportunity if credentials are compromised and ensures that access is periodically re-verified.

- **Global MFA if your IdP does not have MFA enabled**
    
    Enable Multi-Factor Authentication at the NetBird level if your Identity Provider does not enforce MFA. This adds an additional authentication layer to protect against credential theft and unauthorized access.

### 10.2 Settings - Permissions

Configure these permission settings to restrict dashboard access:

Where to configure: **Settings → Permissions** in the NetBird dashboard.

- **Restrict dashboard for regular users enabled**
