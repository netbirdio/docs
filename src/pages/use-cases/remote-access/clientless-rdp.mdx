...
<Note>
RDP also has an optional UDP transport for smoother sessions. It is not
usable through the shared proxy cluster: a UDP service gets its own
automatically assigned listen port, which cannot match the TCP service's
port that the client expects UDP on. Windows clients detect this and fall
back to TCP-only automatically, so no configuration is needed.
</Note>
...
<p>
    <img src="/docs-static/img/use-cases/clientless-rdp/tcp-service-access-control.png" alt="TCP service Access Control tab with CrowdSec IP Reputation set to Enforce and an Allow Only rule for the IP address 203.0.113.40/32" className="imagewrapper"/>
</p>
