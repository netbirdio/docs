...
2. Add an **IP allow rule** for the firm's egress addresses only:
   `203.0.113.40/32`.
3. Set **CrowdSec IP Reputation** to **Enforce** to drop connections from
...
- **No user authentication at the proxy.** SSO, password, and PIN
  ...
  that remain are the IP allowlist, CrowdSec filtering, and the RDP host's
  own login (keep NLA enabled, enforce strong passwords and account
  lockout).
