...
cluster you can bind a TCP and a UDP service to the same custom port, and
the UDP transport then works: Windows clients negotiate it through the
proxy and drop back to TCP-only if the UDP service is removed. In our
testing, the Windows App client on macOS did not offer the UDP transport,
even with the app's UDP setting enabled (the server logs "No Client UDP
Support"), so macOS users connect TCP-only regardless of how the service
is published.
</Note>
