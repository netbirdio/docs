...
| Best for | Fast rollout, few users | Standard ports, smoothest sessions, more control |
...
1. Register and run your proxy per
   [Bring Your Own Proxy](/manage/reverse-proxy/bring-your-own-proxy). For
   RDP the proxy terminates no TLS, so L4 services need no certificate
...
proxy, include the assigned port; for BYOP on port `3389`, the bare
hostname is enough:
