...
Latency-based selection is also not a guarantee of *which* exit node a given user lands on. If a rule must always hold, such as certain users always exiting in a specific country, use separate exit nodes with per-group distribution, or posture checks as in [Geo-Based Exit Node Routing](#geo-based-exit-node-routing) below. Keep their distribution groups from overlapping so that no device receives more than one auto-applied exit node (the failure mode described in step 2 above), and for optional exits that users pick themselves, disable [Auto Apply](#exit-node-selection-and-auto-apply).
