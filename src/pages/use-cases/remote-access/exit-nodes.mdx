...
This policy is all the exit node needs to forward internet traffic; no additional policy to the routing peer itself is required. Also set a DNS server with match domain `ALL` so name resolution follows the tunnel; see [Configure DNS](#5-configure-dns) above.
