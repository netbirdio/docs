...
<Note>
    Build this in **Networks**, not with **Add Exit Node** on the Peers page. An exit node created there pushes its default route to every peer in its distribution groups, and posture checks have no effect on that: a posture check on the access policy only decides whether the exit node accepts a peer's traffic once it arrives. So a device that fails the check still sends all its internet traffic into the tunnel, the exit node drops it, and the device is left with no internet at all rather than a split tunnel.
</Note>
...
On a device outside the US, run `netbird networks ls` and confirm the `internet-egress` network is selected, then check the public IP the device egresses with (for example `curl ifconfig.me`): it should be the exit node's IP.
