...
export const description =
    'Run a private NetBird Reverse Proxy with no public inbound ports by issuing a wildcard certificate with LEGO over DNS-01 and hot-reloading it through a shared volume.'

# Run a Private Reverse Proxy with LEGO

You want your team to open an internal service over HTTPS with no certificate warnings, but the host sits behind NAT or a policy that forbids all public inbound traffic. That's a conflict: a publicly trusted certificate normally requires the certificate authority to reach your proxy on a public port. This guide resolves it. You'll run a NetBird Reverse Proxy that never accepts a connection from the public internet yet still serves a valid, publicly trusted certificate to every authorized NetBird peer.

This setup is a great fit for admin consoles, internal APIs, development environments, and homelab services. As the running example, we'll expose an internal Grafana dashboard at `https://grafana.proxy.example.com`, reachable only by peers in your NetBird network.

The NetBird Reverse Proxy's built-in ACME client uses `tls-alpn-01` or `http-01`. Both of those challenges require the certificate authority to connect to public port 443 or 80. [LEGO](https://go-acme.github.io/lego/) can instead complete a `dns-01` challenge by creating a temporary TXT record through your DNS provider's API. The certificate authority validates DNS and never needs an inbound connection to the proxy.
...
I do recommend pinning both images to tested patch versions in production. This example follows LEGO v5 syntax, where `run` comes before its flags. If you use LEGO v4, follow the [v4-to-v5 migration guide](https://go-acme.github.io/lego/migration/) when adapting commands and hook variables.
