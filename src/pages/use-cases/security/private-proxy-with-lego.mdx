...
printf 'Authorization: Bearer %s\n' "$(<secrets/cloudflare_dns_api_token)" |
  curl -fsS https://api.cloudflare.com/client/v4/user/tokens/verify --header @-
```

The `--header @-` flag makes `curl` read the header from standard input, so the token never appears in the command line or in the process list.
...
Back up both the `lego_state` and `proxy_certs` volumes. The first contains the ACME account and certificate state required for reliable renewals, and the second holds the `wildcard.crt` and `wildcard.key` files the proxy serves. Restoring only one of them leaves you either unable to renew or unable to serve traffic until the next issuance.
