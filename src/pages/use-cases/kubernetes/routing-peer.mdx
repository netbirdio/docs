...
Create a network egress resource in the namespace where it will be consumed. The operator will create a accompanying service with the same name for the egress resource. In this example the target is the nginx deployment from the network resource example.
