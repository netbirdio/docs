...
By default NetBird forwards the caller's identity to the upstream provider so usage and cost
can be attributed outside the dashboard: the **user** and the **group that authorized the
request** are stamped onto each upstream request as provider-specific metadata. For AWS
Bedrock this is the [`X-Amzn-Bedrock-Request-Metadata`](/agent-network/integrations/bedrock#cost-allocation)
header used for AWS cost-allocation tags; other gateways receive their equivalent identity
headers.

Turn on **Disable identity metadata** on a provider to stop sending it. The toggle applies
to every provider and is **off by default** (metadata is sent).
