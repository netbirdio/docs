...
To keep the caller's identity out of the upstream request, turn on **Disable identity
metadata** on the provider — NetBird then stops forwarding the user and group. The toggle is
available for every provider and is **off by default**.
