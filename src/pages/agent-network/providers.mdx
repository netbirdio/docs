...
When you split one gateway into several provider records, for example one record per
model family, the listing includes the models allowed by each matching record. The
records must use the same upstream URL, authentication header and credential, discovery
host, and TLS verification setting. Each record must authorize the caller, and its own
policy restrictions still apply. Inference requests continue to route by model as usual.
