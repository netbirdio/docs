...
- **Cost** — hover to see how much of it was billed for prompt-cache usage.
...
regular input tokens. These buckets are recorded per request and shown on hover
in the **Tokens** and **Cost** columns. Cache writes bill at a premium —
typically 1.25× the input rate — so the first request of a session can cost
noticeably more than its input and output counts alone suggest, while
follow-up requests that read the cache cost less.
...
- **Tokens** and **Cost** — summed across the whole session; hover for the
  breakdown, including the session's prompt-cache tokens and cache cost.
