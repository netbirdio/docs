...
You can set `debug` or `trace` log levels. The log files are available on Linux in `/var/log/netbird` folder and on Windows 
in `C:\ProgramData\netbird` folder.
...
The NetBird applies WireGuard with kernel module by default. With the environment variable you can force to use userspace mode.
...
The NetBird agent communicates with the Managmenet and Signal server with the GRPC framework. With these parameters you can 
...
The Netbird agent communicates with other peers through the Pion ICE library. To debug the connection procedure, 
...
In the case that the peer-to-peer connection is not an option then the peer will use the TURN server for the secure connection establishment.
