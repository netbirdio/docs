...
NetBird is available in the official OpenWrt package feed. The packaged version depends on your OpenWrt release:
...
OpenWrt runs dnsmasq on port 53, which conflicts with NetBird's managed DNS. To use NetBird DNS features such as [DNS routes](/manage/dns/dns-routes) and peer name resolution, run NetBird's resolver on an alternative port and forward NetBird domains to it through dnsmasq.
...
2. **Forward NetBird domains in dnsmasq**
    ...
    Add a server entry to `/etc/config/dhcp` under the `dnsmasq` section, replacing `netbird.cloud` with your NetBird DNS domain if you use a custom one:
...
The OpenWrt package stores NetBird state in `/root/.config/netbird` and preserves it across sysupgrade, so the router stays registered after firmware upgrades. No extra steps are needed.
