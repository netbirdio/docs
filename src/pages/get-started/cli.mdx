...
```
<Note>
    `--disable-firewall` turns off more than access control: routing peer NAT, the DNS port 53 redirect and the userspace packet filter go with it. See [Disable Firewall Configuration](/client/disable-firewall) for what it affects.
</Note>
#### Usage
