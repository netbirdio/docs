...
#### Flags
```shell
      --with-pin string             Protect the exposed service with a PIN
      --with-password string        Protect the exposed service with a password
      --with-user-groups strings    Restrict access to specific user groups
      --with-custom-domain string   Custom domain for the exposed service. Must be configured in your account
      --with-name-prefix string     Prefix for the generated service name
      --protocol string             Protocol to use (default "http", only 'http' is supported)
```
#### Usage
...
```
Expose with PIN protection:
```shell
netbird expose 3000 --with-pin 1234
```
Expose with password protection and a custom name prefix:
```shell
netbird expose 8080 --with-password mysecret --with-name-prefix myapp
```
