...
The `idleTimeout=0` setting disables the idle connection timeout, preventing Traefik from resetting long-lived gRPC streams (such as the signal service) that may appear idle but are actively waiting for messages.
</Note>
