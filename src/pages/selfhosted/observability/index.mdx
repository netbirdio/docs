...
Every server-side component exposes a `/metrics` HTTP endpoint. By default, each listens on port `9090`.
...
The metrics endpoints listen on all interfaces and have no authentication. Restrict access at the firewall, reverse proxy, or service-mesh layer so that only your Prometheus scraper and operators can reach them.
