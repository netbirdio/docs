...
There are a few Identity Provider options that you can choose to run a self-hosted version of NetBird.

<Note>
    NetBird's self-hosted implementation uses the OpenID Connect (OIDC) protocol for authentication, which is an industry-standard identity layer built on top of OAuth 2.0.
</Note>
