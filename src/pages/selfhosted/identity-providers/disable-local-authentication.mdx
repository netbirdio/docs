...
Update your `config.yml` to disable local authentication:

```yaml
...
auth:
    issuer: "https://netbird.example.com/oauth2"
    # Add the following line to disable local authentication
...
After updating the configuration, restart the Netbird Server service. 
...
To restore local authentication, set `localAuthDisabled` to `false` (or remove the option) and restart the Netbird Server service. 
