...
<Note>
    The guide is for new deployments of NetBird with a new identity provider.
    Follow the [Advance guide with a custom identity provider](#advance-guide-with-a-custom-identity-provider) for installations with different IDPs.
</Note>
...
## Advance guide with a custom identity provider
You can use any identity provider that supports the OpenID Connect (OIDC) protocol to manage users in NetBird.
In this guide, we will guide you through deploying NetBird with a custom identity provider for user managem
