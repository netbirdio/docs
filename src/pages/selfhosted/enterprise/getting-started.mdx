...
This serves the certificate on every connection, including clients that send no SNI (Server Name Indication). If your Traefik already has a file provider, add this `tls` block to its existing dynamic configuration and skip the next step's first edit: `providers.file.directory` and `providers.file.filename` are mutually exclusive.
...
Leave the corresponding `traefik.http.routers.*.tls=true` labels in place. On a deployment migrated with `migrate-to-enterprise.sh` the router names are the same, but the `netbird-flow` label sits on the `flow-receiver` service in `docker-compose.override.yml`.
