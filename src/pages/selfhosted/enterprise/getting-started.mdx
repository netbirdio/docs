...
Then delete every router label that references the resolver. A default install has four, but list them rather than counting, so you also catch `docker-compose.override.yml` on a migrated deployment:
