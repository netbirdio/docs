...
**Mount the configuration and the certificates** — add to its `volumes:` list:
...
- **Renewals.** Replace both files at the same paths, then `touch traefik-dynamic.yaml`. Traefik watches that file rather than the certificates it references, so replacing the certificates alone has no effect; touching it reloads them without restarting any container.
