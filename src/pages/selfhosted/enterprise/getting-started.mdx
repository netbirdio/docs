...
This serves the certificate on every connection, including from clients that send no SNI.

### Update `docker-compose.yml`

**Enable the file provider** — add to the `traefik` service's `command:` list:

```yaml
      - "--providers.file.directory=/etc/traefik/dynamic"
```

**Mount the configuration directory and the certificates** — add to its `volumes:` list:
...
…and the four router labels that reference the resolver — one on `dashboard`, two on `netbird-server`, and one on `receiver` if you enabled traffic flow:
