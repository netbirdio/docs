...
If it is `0`, traffic events are not flowing **yet**, and that has nothing to do with relays. Traffic event logging is disabled by default at the account level even when the installer enabled traffic flow on the server: in the dashboard, go to `Settings > Networks` and turn on `Enable Traffic Events` under the Experimental section. See [Traffic Events Logging](/manage/activity/traffic-events-logging). Either enable it and re-run the check, or accept that Step 8 will have nothing to compare against.

## Step 2: Read your existing shared secret

**Do not generate a new secret.** Your deployment already has one, and the traffic-flow receiver uses the same value. Read it from `config.yaml` on the main server:
...
Read it from `config.yaml` and not from `.env`. `config.yaml` carries this value on every deployment. `.env` does not: a deployment created by `getting-started-enterprise.sh` has it there as `NETBIRD_RELAY_AUTH_SECRET`, while one migrated up from the community edition has `NB_FLOW_AUTH_SECRET` instead, because the community installer never wrote the relay secret to `.env` in the first place.
...
<Warning>
`relays.secret` must be the value you read in Step 2, and it must match `NB_AUTH_SECRET` on every relay host. If it does not match, peers will connect normally and traffic event logging will stop. Step 8 checks for exactly this.
</Warning>
...
If it is not, `relays.secret` does not match what the receiver expects. Go back to Step 2, re-read the value from `config.yaml`, and correct `relays.secret` and `NB_AUTH_SECRET` on every relay host to match it. While it is wrong, the receiver log shows:
...
Greater than zero confirms events are arriving. The receiver logs these lines at debug level, which is its default (`NB_FLOW_LOG_LEVEL`). If you have raised its log level, an empty result is not by itself a failure. The `invalid signature` check is the conclusive one, because it is logged as an error.
...
2. `relays.secret` in `config.yaml` on the main server.
3. `server.authSecret` in `config.yaml` on the main server.
4. In `.env` on the main server, under whichever name your deployment uses: `NETBIRD_RELAY_AUTH_SECRET` on a fresh install, `NB_FLOW_AUTH_SECRET` on one migrated from the community edition. That is the value that reaches the traffic-flow receiver.
...
That one is a secret mismatch, and you will see it on a peer only once every relay is rejecting: a peer lists only the relays it is using, so one bad relay among several is simply absent from the list rather than shown as failed. The relay host says so outright either way, in `docker compose logs relay`:
...
Read the secret your deployment already has, build each relay host with it, prove every relay answers from outside, then point `config.yaml` at them and restart `netbird-server` alone. Finally check that the receiver is still accepting flow events, because your peers will look perfectly healthy even when it is not.
