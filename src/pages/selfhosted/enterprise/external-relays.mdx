...
**Do not generate a new secret.** Your deployment already has one, and the traffic-flow receiver, if you have one, uses the same value. Read it from the server configuration on the main server, which is `config.yaml.enterprise` on a migrated deployment that has that file:
...
If it is not, `relays.secret` does not match what the receiver expects. Go back to Step 2, re-read the value from the server configuration, and correct `relays.secret` and `NB_AUTH_SECRET` on every relay host to match it. While it is wrong, the receiver log shows:
