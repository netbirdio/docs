...
Greater than zero confirms events are arriving. These lines are logged at debug level, so an empty result on a deployment that is not logging at debug level is not by itself a failure. The `invalid signature` check is the conclusive one, because it is logged as an error.
