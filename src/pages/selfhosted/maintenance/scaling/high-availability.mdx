...
This example does not configure TLS, so the passwords and all NATS traffic cross the network unencrypted. Keep NATS on a private network, or add TLS as described in the [NATS documentation](https://docs.nats.io/running-a-nats-service/configuration/securing_nats/tls).
