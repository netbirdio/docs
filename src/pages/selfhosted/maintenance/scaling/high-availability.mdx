...
| `NB_AUTH_SECRET` (relay auth) | Update every Relay instance simultaneously, then update `server.authSecret` and `server.relays.secret` on every Management replica at the same time. Peer relay sessions can be rejected while the values differ. Restart Management and Relay together to minimise this window. |
...
- [Scaling Your Self-Hosted Deployment](/selfhosted/maintenance/scaling/scaling-your-self-hosted-deployment) - choose a simpler distributed deployment when active-active Management and Signal are not required
- [Configuration Files Reference](/selfhosted/maintenance/configuration-files) - complete `config.yaml` and dashboard environment reference
- [Back Up a Self-Hosted Deployment](/selfhosted/maintenance/backup) - create and test database backups before infrastructure changes
- [Upgrade a Self-Hosted Deployment](/selfhosted/maintenance/upgrade) - general upgrade guidance for NetBird releases
