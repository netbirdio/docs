...
Remove the `authSecret` from the `server` section and add `relays` and `stuns` sections pointing to your external servers. By default, the presence of the `relays` section disables both the embedded relay and the embedded STUN server, so the `stuns` section is required to provide external STUN addresses. To keep the local services running alongside the external ones, set <code>enableLocalRelay: true</code> and/or <code>enableLocalSTUN: true</code>:
