...
To switch from the embedded relay to external relays, add `relays` and `stuns` sections to your `server` config. By default, the presence of the `relays` section disables both the embedded relay and the embedded STUN server, so the `stuns` section is required to provide external STUN addresses.

If you only want external relays, remove `authSecret` to fully disable the embedded relay:
...
To keep the local services running alongside the external ones, set `enableLocalSTUN: true` and/or `enableLocalRelay: true`. Note that `enableLocalRelay: true` requires `authSecret` to remain configured. The `relays.secret` and `authSecret` must be consistent — if they differ, the embedded relay will reject tokens signed by management with `relays.secret`. When `relays.secret` is omitted, management falls back to `authSecret`, guaranteeing consistency:
...
server:
  ...
  # Keep authSecret — required when enableLocalRelay is true
  ...
  # Force-enable the embedded STUN alongside external STUN
  enableLocalSTUN: true

  # Force-enable the embedded relay alongside external relay
  enableLocalRelay: true
  ...
  relays:
    addresses:
      - "rels://relay-us.example.com:443"
      - "rels://relay-eu.example.com:443"
    # Omit secret so management falls back to authSecret — guarantees consistency
    # secret: "your-shared-secret-here"
    credentialsTTL: "24h"
...
<Warning>
NetBird uses a single token model for relay authentication. The management server signs tokens with <code>relays.secret</code> (or falls back to <code>authSecret</code> when <code>relays.secret</code> is empty), and every relay — embedded or external — validates tokens against the same secret. This means <code>authSecret</code> (embedded relay) and <code>relays.secret</code> (external relay token signing) must resolve to the same value. Either omit <code>relays.secret</code> to rely on the fallback, or set it explicitly to match <code>authSecret</code>.
</Warning>
