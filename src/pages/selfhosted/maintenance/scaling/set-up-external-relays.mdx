...
The proxy must set both `X-Real-Ip` and `X-Real-Port` to the original client's address and source port. The relay accepts those headers only when the immediate connection source matches the trusted list. If either header is missing, the list is empty, or the immediate source is not trusted, the relay ignores the headers and uses the direct connection address.
