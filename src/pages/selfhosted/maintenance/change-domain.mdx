...
- A maintenance window. `docker compose down` in step 4 stops Traefik along with the server, so both hostnames are unavailable for the short time it takes to recreate the stack, with or without the optional routing bridge. The bridge matters after the restart: with it, peers that still point at the old name reconnect; without it, a peer cannot reach Management until you update that peer. Tunnels that are already established generally keep running either way, but a peer cannot receive configuration updates or negotiate new connections until it can reach Management again.
...
If you have many peers, let Traefik accept both names until the last peer has moved. Parenthesize the two `Host()` matchers before combining them with a `PathPrefix`; without the parentheses, Traefik treats every path on the new host as a match for the gRPC and backend routers.
...
The container image runs `netbird up` on start and reads its settings from `NB_*` environment variables. A changed `NB_MANAGEMENT_URL` is written into the stored profile in `/var/lib/netbird` on the next start, so recreating the container with the new value is enough to move the peer.
...
Keep the volume. It holds the peer's WireGuard key, which is what lets it reconnect as the same peer. Once the container is running again, verify the result:

```bash
docker compose exec -T netbird-client netbird status --json
```

The JSON output should report the new `management.url` and `management.connected` as `true`. If your Compose service has a different name, substitute that service name in these commands.
...
Run these commands from its Compose directory to validate the restored settings and recreate the service. The restored `NB_MANAGEMENT_URL` is written into the stored profile on start:
