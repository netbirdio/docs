...
Copy the proxy and CrowdSec files if they are part of the backup:
...
Creating the containers also creates their empty volumes, which you fill in the next step. Leave `crowdsec` out of the command if you do not use it.

```bash
docker compose create netbird-server crowdsec
```
...
If CrowdSec is part of the backup:
