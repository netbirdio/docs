...
<Properties>
  ...
  <Property name="server.enableLocalSTUN" type="boolean">
    Force-enable the embedded STUN server when external STUN servers are configured via <code>stuns</code>. Default: <code>false</code>. Only takes effect when <code>stuns</code> is set. When <code>true</code>, clients receive both local and external STUN addresses.
  </Property>
  <Property name="server.enableLocalRelay" type="boolean">
    Force-enable the embedded relay when external relay servers are configured via <code>relays</code>. Default: <code>false</code>. Only takes effect when <code>relays</code> is set. When <code>true</code>, <code>authSecret</code> remains required. Clients receive both local and external relay addresses.
  </Property>
  ...
  <Property name="server.authSecret" type="string">
    Shared secret for relay authentication. Auto-generated by the setup script. Required when running the embedded relay (no external relay, or <code>enableLocalRelay: true</code>). Can be removed when using external relays without <code>enableLocalRelay</code>. When <code>relays.secret</code> is empty, management falls back to <code>authSecret</code> for token signing — all relays (internal and external) must use the same secret, so either omit <code>relays.secret</code> or set it to the same value as <code>authSecret</code>.
  </Property>
