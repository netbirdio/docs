...
If you are using the standalone `management` container, use the full binary path instead:

```bash
docker compose exec -it management go/bin/netbird-mgmt token create --name "my-proxy"
```
...
For standalone `management` container users:

```bash
docker compose exec -it management go/bin/netbird-mgmt token list
docker compose exec -it management go/bin/netbird-mgmt token revoke <token-id>
```
