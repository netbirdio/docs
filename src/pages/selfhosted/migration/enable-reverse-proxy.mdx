...
### Step 3: Add the proxy service to docker-compose.yml and update the management container gRPC route.
...
Update the management container Traefik route
...
Replace `netbird.example.com` in the `extra_hosts` entry and `traefik` labels  with your actual NetBird management domain. This hairpin NAT fix ensures the proxy can reach Traefik's static IP within the Docker network.
