...
#### 7e. Enable AppSec request inspection (optional)

The same Security Engine can also inspect HTTP requests for exploits (SQL injection, path traversal, known CVE probes). This is a second listener on the CrowdSec container with its own rule set, reusing the bouncer key from step 7b. It applies to HTTP services only.

Create the acquisition file that defines the listener:

```bash
mkdir -p crowdsec/acquis.d
cat > crowdsec/acquis.d/appsec.yaml <<'EOF'
source: appsec
...
Add the rule collections to the CrowdSec service in `docker-compose.yml`, so they are reinstalled if the container is recreated:
...
<Warning>
Both AppSec collections are required. The `appsec-default` config references `crowdsecurity/generic-*` and `crowdsecurity/experimental-*` rules, which only `appsec-generic-rules` provides, and the engine exits at startup if they are missing.
</Warning>

Recreate CrowdSec so it picks up the new environment and acquisition file, then confirm the listener is up:

```bash
docker compose up -d --force-recreate crowdsec
docker compose logs crowdsec | grep -i "appsec listening"
```

Point the proxy at the endpoint by adding one line to `proxy.env` (the API key from step 7b is reused, since the AppSec component validates it against the LAPI):

```bash
NB_PROXY_CROWDSEC_APPSEC_URL=http://crowdsec:7422/
```

Restart the proxy and verify:
...
You should see `CrowdSec AppSec inspection available at http://crowdsec:7422/`. AppSec then appears as a per-service option under **Access Control** alongside IP reputation, off by default. For custom rules, tuning, and testing, see [AppSec (WAF) request inspection](/selfhosted/maintenance/crowdsec#app-sec-waf-request-inspection).

<Note>
Start with **observe** rather than **enforce**. Inspection is synchronous and enforce mode fails closed, so a misconfigured or unreachable engine would deny requests to every service that enforces it.
</Note>

#### 7f. Enroll in CrowdSec Console (optional)
...
See [Step 7: Enable CrowdSec IP reputation](#step-7-optional-enable-crowd-sec-ip-reputation) for the full setup.
