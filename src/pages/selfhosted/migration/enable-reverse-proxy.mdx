...
Add the `AuthCallbackURL` and `AuthClientID` field to the `HttpConfig` section of your `management.json`:

```json
"HttpConfig": {
  ...existing fields...,
  "AuthClientID": "<yourOIDCclientID>",
...
Replace `<yourOIDCclientID>` with the value provided by your IDP.
