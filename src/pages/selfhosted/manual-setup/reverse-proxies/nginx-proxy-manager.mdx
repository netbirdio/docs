...
export const description =
  "Configure Nginx Proxy Manager for manual NetBird combined-container and legacy multi-container deployments with TLS, gRPC, WebSockets, and a private admin UI."
...
This guide covers both the combined and legacy NetBird layouts. Start with [Manual setup](/selfhosted/manual-setup) or [Manual multi-container setup](/selfhosted/manual-setup/multi-container).
...
For combined deployments, also resolve `netbird-server`. For legacy deployments, resolve `netbird-management`, `netbird-signal`, and `netbird-relay`.
...
Put that exact `/32` in both `trustedHTTPProxies` and `trustedPeers`, then restart NetBird Server or the legacy Management service.
