...
export const description =
  "Configure Nginx and Certbot for manual NetBird combined-container and legacy multi-container deployments with gRPC, WebSockets, and trusted proxies."

# Use Nginx with a manual NetBird deployment

This guide installs Nginx on the NetBird host, obtains a Let's Encrypt certificate with Certbot, and routes either the combined or legacy NetBird layout. It also lists the upstream changes for Nginx running in Docker.

Start with [Manual setup](/selfhosted/manual-setup) for the combined layout or [Manual multi-container setup](/selfhosted/manual-setup/multi-container) for the legacy layout.
...
The legacy Compose file from [Manual multi-container setup](/selfhosted/manual-setup/multi-container) publishes all private backends on loopback. Use this Nginx configuration:
...
| Legacy | Dashboard | `netbird-dashboard:80` |
| Legacy | Signal gRPC | `netbird-signal:10000` |
| Legacy | Signal WebSocket | `netbird-signal:80` |
| Legacy | Management | `netbird-management:80` |
| Legacy | Relay | `netbird-relay:80` |
...
For the legacy layout, inspect `signal`, `relay`, or `management` according to the failing path.
