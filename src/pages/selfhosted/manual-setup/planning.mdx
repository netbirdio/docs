...
export const description =
  'Plan a manual self-hosted NetBird deployment, start with the recommended combined server, and add external systems only when you need them.'
...
The [manual combined-container guide](/selfhosted/manual-setup) builds this exact deployment. It is the normal path for a new installation.
...
| ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| [**Combined server**](/selfhosted/manual-setup)                        | One `netbird-server` for Management, Signal, Relay, STUN, and the embedded IdP; separate Dashboard and Traefik containers. | New deployments and installations already migrated to the current architecture. |
| [**Legacy multi-container**](/selfhosted/manual-setup/multi-container) | Separate Management, Signal, Relay, and Dashboard containers.                                                              | Reproducing, repairing, or migrating an older installation.                     |

<Warning>
  Do not choose the legacy multi-container layout for a new long-lived
  deployment. It remains documented so existing installations can be understood
  and migrated safely.
</Warning>

If you have a legacy deployment, follow [Migrate to the combined container](/selfhosted/migration/combined-container) rather than rebuilding the same architecture unless you need it temporarily for recovery or troubleshooting.
...
Each proxy is an integration in front of the same combined NetBird server. Changing the proxy does not change the preferred NetBird architecture.
