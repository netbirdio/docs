...
export const description =
  "Manually deploy NetBird's legacy multi-container Management, Signal, Relay, and Dashboard layout with Docker Compose and an external reverse proxy."

# Manual multi-container setup

This guide manually deploys the legacy NetBird layout with separate `management`, `signal`, `relay`, and `dashboard` containers. It uses the embedded identity provider, SQLite, and the current NetBird images.

<Warning>
New installations should use the [combined `netbird-server` manual setup](/selfhosted/manual-setup). The multi-container layout is retained for existing installations and compatibility testing. If you already run it, plan a [migration to the combined container](/selfhosted/migration/combined-container) rather than treating this layout as the long-term default.
</Warning>
...
- [Traefik multi-container routing](/selfhosted/manual-setup/reverse-proxies/traefik#legacy-multi-container-layout)
- [Nginx multi-container routing](/selfhosted/manual-setup/reverse-proxies/nginx#legacy-multi-container-layout)
- [Caddy multi-container routing](/selfhosted/manual-setup/reverse-proxies/caddy#legacy-multi-container-layout)
- [Nginx Proxy Manager multi-container routing](/selfhosted/manual-setup/reverse-proxies/nginx-proxy-manager#legacy-multi-container-layout)
...
Do not add `--volumes` unless you intend to delete the Management database permanently. Before changes, follow the [backup guide](/selfhosted/maintenance/backup). For the supported long-term layout, follow [Migrate to the combined container](/selfhosted/migration/combined-container).
