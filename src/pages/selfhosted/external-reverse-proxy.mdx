...
export const description =
  "Choose and configure Traefik, Nginx, Caddy, or Nginx Proxy Manager for combined or legacy multi-container self-hosted NetBird."
...
For a new installation, the recommended manual path is the [combined server with its embedded identity provider and Traefik](/selfhosted/manual-setup). Traefik handles automatic certificate issuance and renewal, so you do not need to choose another proxy before NetBird can run.

Use this page when a reverse proxy is an integration choice: you already operate another proxy, need a specific certificate workflow, or maintain a legacy deployment. For the broader architecture decision, start with [Plan a manual NetBird deployment](/selfhosted/manual-setup/planning).

## Choose the NetBird layout

| Layout | Services | Use it when |
|---|---|---|
| **Combined container** | `dashboard` and `netbird-server` | New deployments and migrated installations. This is the current architecture. |
| **Legacy multi-container** | `dashboard`, `management`, `signal`, and `relay` | Maintaining or troubleshooting an older installation before migration. |

- [Manual combined-container setup](/selfhosted/manual-setup)
- [Manual legacy multi-container setup](/selfhosted/manual-setup/multi-container)
- [Migrate a legacy deployment to the combined container](/selfhosted/migration/combined-container)

<Warning>
Do not start a new long-lived production deployment on the legacy layout. The legacy proxy sections exist so older installations can be reproduced, repaired, and migrated safely.
</Warning>
...
| `3478` | UDP | `netbird-server` or legacy `relay` | STUN for NAT discovery. It bypasses the HTTP reverse proxy. |
...
## Legacy multi-container route map
...
The Dashboard catch-all is hiding a missing or lower-priority gRPC route. Run the gRPC probe and inspect the proxy access log. Native gRPC must reach NetBird Server or the correct legacy Signal/Management service over h2c.
...
Inspect the source address seen by the container and configure that exact `/32` in both trust lists. Restart NetBird Server or legacy Management after changing the configuration.
