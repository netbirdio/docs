...
- A public Linux server with at least 1 CPU and 2 GB of memory.
...
`trustedHTTPProxies` trusts only Traefik's fixed address on the private Compose network. If `172.30.0.0/24` overlaps an existing Docker or host network, choose an unused subnet and update both this address and the network settings in `docker-compose.yml`.
