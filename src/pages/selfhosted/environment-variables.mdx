...
| `NETBIRD_CSP` | Space-separated extra origins for the dashboard CSP (`connect-src`, `script-src`, `frame-src`) |
| `NETBIRD_CSP_OIDC_DISCOVERY` | Read the IdP's `.well-known/openid-configuration` at startup and add its endpoint origins to `connect-src`. Default `true` |
| `NETBIRD_CSP_OIDC_DISCOVERY_TIMEOUT` | Seconds to wait for the discovery request. Default `5` |
