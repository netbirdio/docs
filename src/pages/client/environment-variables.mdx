...
| Variable | Platform | Description |
|---|---|---|
| `NB_DISABLE_PROFILE_OWNERSHIP` | Linux, macOS, Windows, FreeBSD | Set to `true` to turn off [profile ownership](/client/profile-ownership). All identified local users can use all profiles and control all connections. Use only on a device with one user or legacy use cases. |
| `NB_DISABLE_DEFAULT_PROFILE_CLAIM` | Linux, macOS, Windows, FreeBSD | Set to `true` to stop the [automatic claim](/client/profile-ownership#automatic-claims) of the default profile. The default profile then has no owner until an administrator runs `netbird profile claim`. |
