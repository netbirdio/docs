...
The same 21 keys apply on every platform. Names are camelCase in the
...
| `allowGroups` | string | Let only the listed groups connect to the daemon socket. Overrides `--allow-group`. An empty value removes the restriction. On macOS, set one `gid:<id>` value. On Windows, set comma-separated `sid:<SID>` values. See [Restrict daemon access to a group](/client/profile-ownership#restrict-daemon-access-to-a-group). |
