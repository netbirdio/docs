...
Every key is enforced on iOS: the ones that reshape the interface, and
the ones the engine applies to the connection itself. Getting there
takes one extra step that is worth understanding, because it decides
*when* a change lands.
...
a bundle. They are enforced just the same — a peer whose policy withholds
`allowRemoteJobs` refuses those jobs — there is simply nothing on screen
to mark yet.
