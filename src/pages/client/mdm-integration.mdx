...
| `disableAutostart` | boolean | Prevent the GUI from registering itself as an OS login-item on fresh installs, and — when enabled at any later point — remove an existing registration on the next GUI launch (Windows Registry `Run` key, macOS Login Item, Linux `.desktop`). Once the admin lifts the policy, the setting stays off until the user re-enables it in Settings. |
