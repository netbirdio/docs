...
4. Upload the resulting `.mobileconfig` to your MDM as a **Custom
   Configuration Profile** and scope it to the target device group.

Profiles delivered through an MDM are signed by the MDM channel itself,
so there is nothing to sign by hand for a managed rollout. A profile you
hand out some other way — Apple Configurator, or a double-click install —
installs unsigned and asks the user for an extra confirmation on recent
macOS releases. To avoid that, sign it as a CMS message:

```bash
security cms -S -N "<signing identity>" -i netbird-macos.mobileconfig -o signed.mobileconfig
```

Use an identity the target Macs already trust. Note that configuration
profiles are **not** signed with `productsign`, which handles installer
packages, nor with an Apple Developer ID certificate.
...
Every key that applies to the platform is enforced on iOS: the ones that
reshape the interface, and the ones the engine applies to the connection
itself. (A handful have no iOS surface at all and are listed at the end
of this section.) Getting there takes one extra step that is worth
understanding, because it decides *when* a change lands.

iOS delivers the configuration to the NetBird app. The tunnel, though,
runs in a Network Extension — a separate process the system starts when
the tunnel comes up and stops when it goes down, and which Apple's
app-configuration channel does not deliver to. The app therefore passes
the configuration on to the extension, and does so whenever it is
running: on activation, when a settings screen appears, and on a timer
roughly every 30 seconds. When the policy has changed, the engine
restarts on its own and the user is told their IT policy was applied.

<Note>
**A policy lands while the app is open.** With NetBird in the
foreground, a change pushed from your MDM reaches the tunnel within
about 30 seconds, or immediately when the user switches back to the
app. Nothing needs to be restarted by hand.

**While the app is closed or backgrounded, iOS suspends it**, and a
policy pushed in that window — or withdrawn in it — reaches the tunnel
only the next time the app is opened. This matters most with VPN On
Demand or the widget, where the tunnel can come up without the app ever
appearing: it then runs on the last policy the app saw. Plan rollouts
accordingly, and treat "applied" as "applied once the user next opens
the app".
</Note>

<Warning>
**On tvOS only the interface is enforced.** The hand-off described above
relies on a shared app group, which does not work between the tvOS app
and its extension, so the tvOS engine runs without the policy. Keys that
lock or hide controls behave as described; the ones the engine applies
to the connection — `managementURL`, `preSharedKey`, `rosenpassEnabled`,
`rosenpassPermissive`, `blockInbound`, `wireguardPort`, `lazyConnection`
— do not reach the tunnel there yet.
</Warning>
...
The table above is what the user sees. A second group of keys has no
control to lock, because the engine applies them straight to the
connection: `blockInbound`, `wireguardPort`, `lazyConnection`,
`allowRemoteJobs`, and `debugBundleUploadURL` when a remote job produces
a bundle. **On iOS** they are enforced just the same — a peer whose policy
withholds `allowRemoteJobs` refuses those jobs — there is simply nothing
on screen to mark. On tvOS none of them reach the engine, for the reason
in the warning above.

The rest do not apply to the platform and are parsed and ignored:
`allowServerSSH` (no SSH server on iOS), `disableServerRoutes` (an iOS
peer does not route for others), `disableAutostart` (desktop GUI only),
`disableMetricsCollection`, `enableLocalMetrics` and
`localMetricsAddress` (the local endpoint is not reachable on iOS), and
`splitTunnelMode` / `splitTunnelApps` (Android). They are safe to leave
in a payload shared with desktop devices.
...
That covers delivery to the app. To confirm a change reached the
*tunnel*, change a policy while the tunnel is connected and bring
NetBird to the foreground: the engine restarts and the device shows
"NetBird configuration was updated by your IT policy". That notice comes
from the extension, so seeing it means the engine applied the new
policy.
...
**On iOS, a policy change has not reached the tunnel.**
The app passes the configuration to the engine only while it is running.
Open NetBird and leave it in the foreground for half a minute; the
engine restarts and reports that the policy was applied. On tvOS the
engine does not receive the policy at all — see [When a policy takes
effect](#when-a-policy-takes-effect).
