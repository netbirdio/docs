...
The tray menu also provides profile switching, session-renewal access, exit-node selection, Settings, Help & Support, and **Quit NetBird**. Closing the main window only hides it while the daemon and network connection continue running. Choosing **Quit NetBird** disconnects the daemon before the desktop app exits. To keep the connection running in the background after quitting, enable **Stay Connected After Quitting** in **Settings → General** (requires client v0.76.2 or later). It is off by default, and with it on the connection only stops when you disconnect it yourself.
