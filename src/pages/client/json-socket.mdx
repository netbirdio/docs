...
caller, so it refuses every request on a TCP socket. Use a Unix socket.
...
gateway socket the daemon refuses every request.

### A TCP Request Is Refused

TCP sockets are deprecated, and the daemon refuses every request on them. Change `--json-socket` to a Unix socket. See [TCP sockets (deprecated)](#tcp-sockets-deprecated).
