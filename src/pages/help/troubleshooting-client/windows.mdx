...
Windows-specific steps for the NetBird client. For everything cross-platform (agent status, the debug bundle, GRPC and ICE debugging, login failures, and reaching resources), start from [Troubleshooting client issues](/help/troubleshooting-client).

## Set the log level permanently

The [temporary log level](/help/troubleshooting-client#enabling-debug-logs-on-agent) resets when the service restarts. To make it permanent, run an elevated PowerShell or `cmd.exe` window:
...
On Windows the agent depends on WireGuard's `wintun.dll` and can only run as the system account. To run it in foreground mode, use [PSExec](https://learn.microsoft.com/en-us/sysinternals/downloads/psexec). In an elevated PowerShell window:
...
To pass environment variables, set them as machine-level variables so the agent picks them up on the next PSExec run:
