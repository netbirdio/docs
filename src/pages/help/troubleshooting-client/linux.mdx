...
Linux-specific steps for the NetBird client. For everything cross-platform (agent status, the debug bundle, GRPC and ICE debugging, login failures, and reaching resources), start from [Troubleshooting client issues](/help/troubleshooting-client).

## Set the log level permanently

The [temporary log level](/help/troubleshooting-client#enabling-debug-logs-on-agent) resets when the daemon restarts. To make it permanent on Linux, use one of the following.
