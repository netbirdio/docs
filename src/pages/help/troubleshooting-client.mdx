...
<TroubleshootingStart
  ...
  steps={[
    { label: "1. Check status", title: "Agent status", href: "#net-bird-agent-status", command: "netbird status -d", hint: "Is the peer Connected, and are Management, Signal, and the Relays reachable?" },
...
<TroubleshootingTiles
  ...
  items={[
    ...
    },
    {
      ...
      chips: [
        { label: "Foreground (PSExec)", href: "/help/troubleshooting-client/windows#run-the-agent-in-foreground-mode" },
...
You can run the agent in foreground mode to see the logs in the terminal. This is useful when debugging issues with the agent. On Linux and macOS:
...
On Windows, foreground mode needs PSExec because the agent runs as the system account. See [Run the agent in foreground mode](/help/troubleshooting-client/windows#run-the-agent-in-foreground-mode) on the Windows page.
