...
* **Upload to NetBird** uploads the completed archive to NetBird's fixed debug-bundle service. Disable it to create a local archive only.
...
Click **Create debug bundle**. You can cancel while logs are being collected. When the operation finishes, the app displays the upload key when upload was enabled and keeps the local file path available so you can reveal the archive in the file manager.

<p>
    <img src="/docs-static/img/help/troubleshooting-client/ui-bundle-success.png" alt="Completed debug bundle in the NetBird desktop app" className="imagewrapper-big"/>
</p>

Copy the upload key and share it with NetBird support, or attach the local archive through the support channel you are using.
