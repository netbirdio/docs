...
1. The management server sends a job request to the target peer via gRPC
...
To trigger a remote debug bundle generation via the API, send a POST request to the peer debug bundle endpoint:

```bash
curl -X POST https://api.netbird.io/api/peers/{peerId}/debug-bundle \
...
The peer must be online and connected to receive the job. Once the bundle is generated and uploaded, you'll receive
a job ID that can be used to check the status and retrieve the upload key.

To list all jobs for a peer:
...
See the [Peers API documentation](/ipa/resources/peers#create-peer-debug-bundle) for complete API reference.
