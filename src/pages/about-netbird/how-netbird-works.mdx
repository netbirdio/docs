...
After installation, NetBird client requires login that can be done through Identity Provider (IDP) like Okta or with a [setup key](/manage/peers/setup-keys).
...
The client application requests a user to log in with an Identity Provider (IDP) or a [setup key](/manage/peers/setup-keys) so that the peer can be associated with the organization's account.
