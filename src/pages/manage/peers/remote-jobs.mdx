...
The destination follows the [precedence order](/manage/settings/clients#debug-bundle-upload): the peer's MDM policy first, then this job's **Upload URL** if you fill it in, then the account setting under Settings > Clients, then the management server's own default, and the upload service NetBird runs only when none of those is set.
