...
This guide introduces [NetBird's Setup Keys](/manage/peers/setup-keys) as an elegant solution for seamlessly and securely adding servers and containers to your network by:
...
Let's dive into the step-by-step process of using [NetBird's Setup Keys](/manage/peers/setup-keys) to securely add a server with a Docker container to your network.
...
To seamlessly integrate virtual machines or Docker containers into your NetBird network, you'll start by creating a setup key. While [NetBird's documentation offers comprehensive guidance on this process](https://docs.netbird.io/manage/peers/setup-keys), let's quickly review the essential steps:
