...
To enable client auto updates, navigate to [Settings &raquo; Clients](https://app.netbird.io/settings) and select an update method under **Automatic Updates**:
...
2. If the Peer is unable to download or install the update (e.g., due to network issues), it will show an error notification. Forced updates are retried automatically on subsequent update checks; prompted updates can be retried by the user from the tray menu.
