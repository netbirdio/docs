...
3. **Access Control Policy** - either:
   - A `NetBird SSH` policy for fine-grained access control (recommended), or
   - A TCP policy allowing port 22 for basic network-level SSH access
...
<Note>
    The `NetBird SSH` protocol option provides fine-grained access control features not available with standard TCP policies. When using `TCP` with port 22, the management server automatically adds rules for port 22022 (NetBird's internal SSH port) for peers running v0.60.0+.
</Note>
...
4. **Fine-grained access control** (when using NetBird SSH protocol): Controls which authenticated users can access which local OS users on the target machine
...
<Note type="warning">
    **Breaking change**: Management server v0.61.0 will break NetBird SSH for all peers running versions older than v0.61.0. You must update all peers that use SSH to v0.61.0 or later before upgrading the management server.
</Note>
...
**Upgrading from v0.60.x:**

<Note type="warning">
    **Update order is critical:**
    1. Update all SSH server peers to v0.61.0 first
    2. Update all SSH client peers to v0.61.0
    3. Update the management server to v0.61.0 last

    Peers running older versions will lose SSH functionality once the management server is upgraded.
</Note>

- Existing TCP port 22 policies will continue to work for network-level access
- To use fine-grained access control, create new policies with the `NetBird SSH` protocol
- Fine-grained policies support both **Full Access** (any local user) and **Limited Access** (specific local users per group)
