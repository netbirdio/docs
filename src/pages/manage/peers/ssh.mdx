...
### Configuring Authorized Groups

When creating or editing an SSH policy in the Dashboard:

1. Select `NetBird SSH` as the protocol
2. Configure the source groups (NetBird users who will connect)
3. Configure the destination (peers running SSH server)
4. In the **Authorized Groups** section, map each source group to the local OS users they can access
...
- No breaking changes - existing SSH policies continue to work
- To use fine-grained access control, create new policies with the `NetBird SSH` protocol
- Existing TCP port 22 policies provide network-level access without user mapping
