...
<Note type="warning">
    **Update order is critical** - management must be updated first, followed by dashboard, and then peers.
</Note>

1 . **Update the management server** to v0.61.0+

2. **Update all SSH server peers** to v0.61.0+ first (peers running `--allow-server-ssh`)
   - Start with non-critical peers for testing

3. **Update all SSH client peers** to v0.61.0
