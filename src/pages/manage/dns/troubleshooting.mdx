...
- **The port 53 redirect.** The local resolver prefers port 53, but when another DNS server on the host already holds it the resolver falls back to another port and the client redirects port 53 to it with firewall rules. Without the firewall the redirect is missing and queries keep going to port 53. Confirm with `sudo ss -ulnp | grep :53` on the peer: NetBird listening on a port other than 53 means the redirect was needed.
