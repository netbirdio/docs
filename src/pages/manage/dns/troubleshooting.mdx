...
# Use these exact tools: dig, host, and nslookup test a DIFFERENT path
...
Search domains sharpen the trap: they *do* merge into `resolv.conf`, so `host myserver` expands the short name to `myserver.company.internal` and then still asks the public resolver.
