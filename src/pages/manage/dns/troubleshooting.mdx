...
**Symptoms**:
- `nslookup internal.company.local` fails
- `ping server.company.local` returns "unknown host"
...
2. **Check match domains**:
   ...
   - `company.local` won't match `*.company.local` (need both)
...
// Doesn't match "company.local" (no subdomain)
...
dig @10.0.0.1 internal.company.local
...
for i in {1..10}; do 
  echo "Test $i:"
  time nslookup internal.company.local
  sleep 1
done
...
time dig @10.0.0.1 internal.company.local
time dig @10.0.0.2 internal.company.local
...
- `ping server.company.local` does work
...
# If still shows 100.0.0.x, NetBird didn't restore original DNS
```
...
- ✅ Include both domain.local and *.domain.local patterns
...
- **[DNS Overview](/manage/dns/overview)** - Understand DNS architecture
