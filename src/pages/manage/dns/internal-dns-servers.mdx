...
- **Why a domain resource isn't enough** — the routing peer serves only `A`/`AAAA`, not `SRV` / DC-locator records; and because it's now the resolver for the domain, the client treats its answers as authoritative and won't fall back to the DC for the missing records.
