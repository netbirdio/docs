...
With [Routing Peer DNS Resolution](/manage/networks/how-routing-peers-work#routing-peer-dns-resolution) on, a domain resource is enough for a plain file share — the routing peer resolves its name-to-IP (`A`/`AAAA`) lookup. Active Directory needs more. AD depends on `SRV` / DC-locator records for logon and DFS, which the routing peer does **not** serve; and because the routing peer is now the resolver for the domain, the client treats its answers as authoritative and won't fall back to the DC for the records it's missing. So a domain-joined client needs a **nameserver** (`corp.example.com` → the DC) — and it only works once the DC itself is reachable through the peer. This is the same on Windows and Linux clients.
