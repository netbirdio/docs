...
When the DC runs no agent and remote domain-joined clients reach it **through** another routing peer (for example, a file server that runs the agent), those clients still need a **nameserver** (`corp.example.com` → the DC). A domain resource alone is not enough. With [Routing Peer DNS Resolution](/manage/networks/how-routing-peers-work#routing-peer-dns-resolution) on, the routing peer answers ordinary name-to-IP (`A`/`AAAA`) lookups — but Active Directory also needs the `SRV` / DC-locator records for login and DFS, which the routing peer does **not** serve. The nameserver only works once the DC itself is reachable (routed through the peer). This is the same on Windows and Linux clients.
