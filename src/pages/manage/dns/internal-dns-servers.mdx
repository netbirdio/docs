...
A domain resource alone is not enough for Active Directory. With [Routing Peer DNS Resolution](/manage/networks/how-routing-peers-work#routing-peer-dns-resolution) on, the routing peer answers ordinary name-to-IP (`A`/`AAAA`) lookups — but AD also needs the `SRV` / DC-locator records for login and DFS, which the routing peer does **not** serve. So a domain-joined client still needs a **nameserver** (`corp.example.com` → the DC), and that nameserver only works once the DC itself is reachable through the peer. This is the same on Windows and Linux clients.

For the full setup — routing the DC, the policies, DFS, and verification — see [Active Directory & Windows File Shares](/manage/networks/use-cases/active-directory).
