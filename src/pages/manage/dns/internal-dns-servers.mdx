...
- **Active Directory.** Kerberos and DFS need more than plain name-to-IP lookups — they rely on **SRV records** (the "where is a domain controller?" entries Windows uses to find the DC), which the routing peer does not answer. A domain-joined client needs a nameserver (`corp.example.com` → the DC) no matter how this setting is configured. See [Reaching a Domain Controller through a routing peer](#reaching-a-domain-controller-through-a-routing-peer).
</Note>
...
<Note>
Setting up remote access to AD file shares, DFS, and domain login end to end? See the [Active Directory & Windows File Shares](/manage/networks/use-cases/active-directory) guide — it ties together the routing, DNS, and Kerberos pieces into one recipe. The sections below are the DNS reference behind it.
</Note>
...
When the DC runs no agent and remote domain-joined clients reach it **through** another routing peer (for example, a file server that runs the agent), those clients still need a **nameserver group** (`corp.example.com` → the DC). A domain resource alone is not enough: with [Routing Peer DNS Resolution](/manage/networks/how-routing-peers-work#routing-peer-dns-resolution) on, the routing peer answers ordinary name-to-IP (`A`/`AAAA`) lookups, but Active Directory also needs the `SRV` / DC-locator records for login and DFS — which the routing peer does **not** serve. The nameserver group only works once the DC itself is reachable (routed through the peer). This is the same on Windows and Linux clients.

For the complete setup — routing the DC, the policies, DFS, and verification — see [Active Directory & Windows File Shares](/manage/networks/use-cases/active-directory).
