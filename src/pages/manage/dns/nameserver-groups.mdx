...
- [DNS basics and how NetBird handles DNS](/manage/dns/overview)
...
**Supported patterns**:
- Exact match: `company.local`
- Wildcard subdomain: `*.company.local` (matches `app.company.local` but NOT `company.local` itself)
...
<Note>
**Tip**: Include both `domain.local` and `*.domain.local` to match the apex domain and all subdomains.
</Note>
...
When enabled:
- Typing `server` expands to `server.company.local`
...
**Goal**: Use Cloudflare for internet, internal DNS for `*.company.local`
...
```json {{ title: 'Match - Internal' }}
{
  ...
  "enabled": true,
  "primary": false,
  "domains": ["company.local", "*.company.local"],
...
- `app.company.local` → Internal DNS (10.0.0.1)
- `server` → Expanded to `server.company.local` → Internal DNS
...
1. **For Developers**:
   ...
   - Match: Internal DNS (*.company.local)
...
```
Query: app.us-east.company.local

Nameservers:
1. *.company.local → DNS1
2. *.us-east.company.local → DNS2
...
```
company.local → On-premise Active Directory
*.corp.company.local → Corporate services
...
ping server  # Should expand to server.company.local
```
