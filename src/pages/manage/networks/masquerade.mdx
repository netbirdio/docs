...
1. **Give the routing peers different metrics**, so the active peer is the same one for every client. With equal metrics each client picks its own by latency, and then no single next hop is correct for all of them. See [High availability](/manage/networks/how-routing-peers-work#high-availability). Networks group the routers of a resource for you. If you still use the deprecated Routes, the two routes must share one network identifier and range, or their metrics are never compared: see [High Availability](/manage/network-routes#high-availability) on the Routes page.
