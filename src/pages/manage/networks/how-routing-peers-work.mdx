...
Forwarding to machines *behind* the routing peer (routed subnets) never needs this flag. It matters only for reaching a service on the routing peer's **own address** — the self-access case — and only when the peer forwards in software (the userspace router):
...
*† In netstack mode, `NB_ENABLE_NETSTACK_LOCAL_FORWARDING` is a netstack-specific override; it takes precedence if both are set.*
...
- **At its LAN IP** — when DNS (or a DFS path) resolves the service to that address, the client has **no route** to that IP until you add a **resource** that covers it. So you need *both* a resource (the route) *and* a peer-to-peer policy (the input-chain accept) — **and, on userspace peers, [`NB_ENABLE_LOCAL_FORWARDING`](#ip-forwarding)**.
