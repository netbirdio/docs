...
Leave it off otherwise. With it on, any permitted peer can reach services bound to the routing peer's own addresses, not just the networks behind it — so don't enable it on a peer running local-only services you don't want exposed.
