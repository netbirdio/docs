...
With forwarding in place, no further setup is needed to reach the networks *behind* the peer. Reaching a service bound to the routing peer's own address — its LAN IP or loopback, not the network behind it — is different. On any peer that forwards in userspace (Windows and macOS always; Linux in netstack mode), it requires `NB_ENABLE_LOCAL_FORWARDING`; Linux kernel-mode peers deliver to their own address without it. The flag is off by default on every platform (see [Client Environment Variables](/client/environment-variables)):

```powershell
netbird service reconfigure --service-env NB_ENABLE_LOCAL_FORWARDING=true
```

The toggle alone is not enough. To reach the peer at its own LAN IP, clients also need a **route** to that address (a resource covering the peer's own IP) and a **peer-to-peer policy** (input chain). All three are required together — see [Reaching the routing peer itself](#reaching-the-routing-peer-itself). This is the common shape when the service host *is* the routing peer: a file server that runs the agent, whose clients resolve its name to its LAN IP.

Leave it off otherwise. With it on, any permitted peer can reach services bound to the routing peer's own addresses — including `127.0.0.1` — at the peer's NetBird IP, unintentionally exposing localhost-only services.
...
There are two ways to address the routing peer, and they need different things:

- **At its NetBird (overlay) IP** — a single peer-to-peer policy (input chain) is enough. Every client already has an overlay route to every peer.
- **At its own LAN IP** — when DNS (or a DFS path) resolves the service to that address, the client has **no route** to that IP until you add a **resource** that covers it. So you need *both* a resource (the route) *and* a peer-to-peer policy (the input-chain accept) — **and, on userspace peers, [`NB_ENABLE_LOCAL_FORWARDING`](#ip-forwarding)**.

A resource that covers the peer's own IP can be a `/32` of that IP, a subnet that contains it, or a **domain resource equal to the peer's own FQDN** (the peer resolves its own name to its LAN IP and the route is installed for you — the cleanest option, no hardcoded address).

<Note>
Test this path with `Test-NetConnection <host> -Port <port>` (or `nc`), not `ping`. ICMP needs its **own** peer-to-peer policy; a service working while `ping` fails is the signature of a missing ICMP policy, not a broken route.
</Note>
