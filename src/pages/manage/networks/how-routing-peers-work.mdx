...
- **At its LAN IP** — when DNS (or a DFS path) resolves the service to that address, the client has **no route** to that IP until you add a **resource** that covers it. So you need *both* a resource (the route) *and* a peer-to-peer policy (the input-chain accept) — **and, on userspace peers, [`NB_ENABLE_LOCAL_FORWARDING`](#local-forwarding)**.
