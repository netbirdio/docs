...
Routing peer DNS resolution moves DNS resolution for routed domains from the client to the routing peer. Wildcard domain resources require it.

<Note>
With routing peer DNS resolution enabled, the routing peer handles DNS lookups for the network's domain resources, enabling access control rules and optimized traffic routing.
</Note>

To enable it:

1. Navigate to `Settings` > `Networks` in NetBird
2. Toggle `Enable Routing peer DNS resolution`

![Enabling routing peer DNS resolution](/docs-static/img/manage/networks/use-cases/by-resource-type/accessing-entire-domains-within-networks/01-domains-within-networks.png)

<Note>
Routing peer DNS resolution requires routing peers and clients running version `0.35.0` or later. Restart your routing peers and clients after enabling this feature.
</Note>
