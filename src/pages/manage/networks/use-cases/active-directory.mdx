...
The DC is almost always agentless and reached *through* the routing peer. Add it as a resource in the NetBird Network:
...
Keep it granular — least privilege is the NetBird default, so route only what you need. A single `*.corp.example.com` wildcard is the shortcut: it covers the DC, file servers, and DFS targets in one resource. But a wildcard is one resource with **one policy scope** — the ports you open apply to *every* host under the domain, so you can't give the DC the full AD port set while limiting file servers to `445`. Use separate resources when you want that per-host scoping.
