...
export const description = "Choose the size and number of NetBird routing peers from measured throughput: a four-step method, a per-peer capacity table, the tuning levers that matter, and how to scale out across multiple Networks."
...
|---|---|---|
| 1280 (default) | ~31 Gbps | ~15.5 Gbps |
| 8920 (jumbo) | ~95 Gbps | ~66 Gbps |

That is roughly **3x on download and 4x on upload** from one setting. At jumbo the download direction reached ~95 Gbps, essentially the 100 GbE interface's line rate, so the interface capped it before the CPU did; the upload direction still ran about 29% idle, so it had further left to climb. The practical result is that a single 16-vCPU-or-larger peer on a jumbo path can approach interface line rate, which cuts the peer count sharply: carrying 100 Gbps of downloads drops from four or five default-MTU peers to one or two.
