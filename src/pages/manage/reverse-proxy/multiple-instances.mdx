...
Running a single proxy instance works well for many deployments, but production environments often require higher availability. By deploying multiple proxy instances across separate servers, you get automatic failover - if one instance goes down, the remaining instances continue serving traffic without interruption.
...
Generate a token for each instance:

```bash
docker exec -it netbird-server netbird-mgmt token create --name "proxy-server-1"
docker exec -it netbird-server netbird-mgmt token create --name "proxy-server-2"
```
...
# Revoke a specific instance's token
...
# Revoke its token on the management server
docker exec -it netbird-server netbird-mgmt token list
docker exec -it netbird-server netbird-mgmt token revoke <token-id>
```
