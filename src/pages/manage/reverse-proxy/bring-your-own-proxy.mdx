...
| **Private** | At least one connected proxy in the cluster runs embedded in a NetBird client (`netbird proxy`) and serves over the WireGuard tunnel. Required to enable [NetBird-Only Access](/manage/reverse-proxy/authentication#netbird-only-access-private-services) and the **Proxy Cluster** target type on services. |
