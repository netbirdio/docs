...
It uses the rootless proxy image built on Red Hat Universal Base Image (UBI) 9, published for Linux AMD64 and ARM64 as `ghcr.io/netbirdio/reverse-proxy:0.80.0-rootless-ubi`. The image runs as a non-root user and keeps its writable directories group-writable so OpenShift can assign an arbitrary UID. Pin a version or digest for reproducible deployments; see [NetBird releases](https://github.com/netbirdio/netbird/releases).
...
export NB_SERVICE_DOMAIN=nginx.proxy.example.com
...
The Deployment reads `NB_PROXY_TOKEN` and `NB_PROXY_DOMAIN` from the `proxy-token` and `proxy-domain` keys of a Secret named `netbird-proxy-config`. Create it with your proxy access token:
