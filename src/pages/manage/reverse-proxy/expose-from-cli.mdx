...
4. Optionally, select specific **peer groups** that are allowed to expose services. If no groups are selected, all connected peers can expose services.
...
|------|------|---------|-------------|
| `--protocol` | string | `http` | Protocol to use. Only `http` is currently supported. |
| `--with-pin` | string | | Protect the exposed service with a PIN code. |
| `--with-password` | string | | Protect the exposed service with a password. |
| `--with-user-groups` | strings | | Restrict access to specific user groups (SSO authentication). Comma-separated. |
| `--with-custom-domain` | string | | Use a custom domain instead of an auto-generated one. Must be pre-configured in your account. |
| `--with-name-prefix` | string | | Prefix for the generated service name and subdomain. |
...
Protect the service with a PIN code that users must enter before accessing it:

```shell
netbird expose 8080 --with-pin 1234
```
...
```shell
netbird expose 8080 --with-pin 1234 --with-password backup-password --with-user-groups admin
```
