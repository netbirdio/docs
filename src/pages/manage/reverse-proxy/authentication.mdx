...
2. The owning user's groups are checked against the service's **access groups**. If the user is not in any of them, the request is rejected.
...
Enabling NetBird-Only Access applies an implicit allow rule for the NetBird network range on the **Access Control** tab — additional CIDR, country, or CrowdSec rules you add are layered on top of that baseline. The Access Control tab shows an informational callout when NetBird-Only is on.
