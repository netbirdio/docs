...
| **TCP** | L4 | Raw TCP relay. The proxy accepts TCP connections on a port and forwards them to your backend. |
| **UDP** | L4 | UDP relay with session tracking. The proxy accepts UDP packets on a port and forwards them to your backend. Sessions are reaped after an idle timeout. |
...
L4 services (TCP, UDP, TLS) listen on a port on the proxy cluster. Several services can share a port: TLS services are distinguished by their SNI hostname, and each port takes at most one TCP and one UDP service (see [How services share ports](#how-services-share-ports)). Depending on the cluster, the port may be auto-assigned or you can specify one manually. The proxy cluster's `supports_custom_ports` capability determines whether manual port selection is available.
...
L4 services (TCP, UDP, TLS) listen on a port on the proxy cluster. How ports are assigned depends on whether the proxy cluster supports custom port selection:
...
<Warning>
    A port on a proxy cluster can be shared by any number of TLS services, which are distinguished by their SNI hostname, and at most one TCP service, which receives every connection that doesn't match a TLS service (or, on the main port, an HTTP service). UDP is counted separately: each port takes at most one UDP service, which can use the same port number as TCP and TLS services. Creating a service fails if its port already has a service of the same protocol (a second TCP or UDP service), or if another service already uses its domain. L4 listen ports also cannot conflict with the proxy's tunnel port.
</Warning>
...
- If the SNI hostname does not match any service, the connection goes to the TCP service on that port if there is one, and otherwise falls through to the HTTP reverse proxy, which returns an error since no matching service exists.
...
**On custom ports**, any number of TLS services and at most one TCP service can share the same port:
