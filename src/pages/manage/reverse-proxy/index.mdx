...
Multiple proxy instances configured with the same `NB_PROXY_DOMAIN` value form a single proxy cluster, providing automatic failover. See [Running Multiple Proxy Instances](/manage/reverse-proxy/multiple-instances) for setup instructions covering token management, TLS certificates, and monitoring across instances.
...
- [Multiple Instances](/manage/reverse-proxy/multiple-instances) - deploy multiple proxy instances for high availability and redundancy
