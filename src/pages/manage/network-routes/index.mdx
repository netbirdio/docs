...
Connected peers select a routing peer based on metric priority first. Among routing peers with the same metric, they prefer a direct connection over a relayed one, then lower latency.
...
<Warning>
If the two routes use different network identifiers, such as `site-b-primary` and `site-b-standby`, clients treat them as two separate routes for the same range rather than one highly available route. The metric is never compared, each client can end up sending traffic through either routing peer, and which one it uses can change whenever the routes are updated. With masquerade off this breaks traffic in both directions, because replies return through the other routing peer and are dropped.
</Warning>
...
If the same range appears under two different IDs, both marked `Selected`, the routes are not being treated as one highly available route. Give them the same network identifier.
