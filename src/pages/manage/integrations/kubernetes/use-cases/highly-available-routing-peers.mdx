...
With default scheduling, those replicas can all land on the same node — so a single node failure still takes down every routing peer:

<p>
  <img src="/docs-static/img/manage/integrations/kubernetes/ha-routing-peers-single-node.svg" alt="Three routing-peer pods running on a single Kubernetes node, fronting a service for a NetBird client" className="imagewrapper-big"/>
</p>

[Spread them across nodes](#spread-across-failure-domains) to remove that single point of failure.
