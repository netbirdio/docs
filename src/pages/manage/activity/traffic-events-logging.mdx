...
On Linux, you can force a routing peer into userspace mode with two [environment variables](/client/environment-variables):

```bash
sudo netbird service reconfigure --service-env NB_WG_KERNEL_DISABLED=true,NB_FORCE_USERSPACE_ROUTER=true
```
