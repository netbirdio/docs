...
Leaving it empty means the account publishes no destination. Peers then fall through to whatever is next in the list below, and to the service NetBird runs only when nothing else is set — on a self-hosted deployment as much as on NetBird Cloud. That keeps the everyday flow working: collect a bundle, get an upload key, share it with support. If the bundles must not leave your own infrastructure, set this to your own upload service; until you do, they go to NetBird.
