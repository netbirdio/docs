...
The toggle maps to the `regular_users_view_blocked` field on account settings. You can read it with a `GET` and change it with a `PUT` on the account. See the [Accounts API reference](/api/resources/accounts) for the full schema.
