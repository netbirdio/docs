...
Go to the Web UI `Settings` tab and set the desired period in the `Authentication` tab using the `Peer Session Expiration` toggle.
...
This is useful for tightly controlled environments where a device that goes offline — e.g., a laptop that is shut down or leaves the network — should prove the user's identity again before rejoining.
