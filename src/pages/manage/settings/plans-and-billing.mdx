...
In addition to the Stripe billing portal, you can view your invoices directly in the NetBird dashboard. Navigate to `Settings` > `Invoices` to see all available invoices and download them as PDF or CSV files. Invoices are created at the end of each billing period; PDF downloads redirect through Stripe, while CSV files download directly from the dashboard. For MSPs and distributors, the `Type` column distinguishes invoices for your own account from those for your tenants or customers. The `Invoices` tab is not shown for accounts managed by an MSP.
