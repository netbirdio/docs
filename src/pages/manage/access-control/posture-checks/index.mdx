...
Android clients report their local network interfaces from NetBird client [0.77.1](https://github.com/netbirdio/netbird/releases) onward, so ranges intended to match a peer's **local NIC IPs** (e.g. an office LAN like `192.168.1.0/24`) are evaluated on Android exactly as they are on desktop. No configuration is needed beyond running that version or newer.

Before 0.77.1, the Android client never reported those addresses, so local NIC ranges could not be evaluated on the platform at all.
...
- An **Allow** check built on local NIC ranges is never satisfied by such a peer. Those peers fail the check, and routes gated by that policy become unavailable to them.
