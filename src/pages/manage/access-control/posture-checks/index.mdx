...
If a policy's Peer Network Range posture check relies on local NIC ranges, the check cannot match on Android clients, and routes gated by that policy may become unavailable on those devices even when the device is outside the blocked local range.
