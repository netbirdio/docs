...
In these cases you can match the peer's **public egress IP** instead — the source IP the NetBird management server observes when the peer connects. The Peer Network Range posture check evaluates this IP automatically, so the configuration is the same as before; you just put your office's public IP in the range list as a single-host prefix.
...
### IPv6 example
If your office also (or only) has an IPv6 egress, add the address as a `/128`, for example `2001:db8::1/128`. You can include both the IPv4 and IPv6 entries in the same posture check; a peer matches if its connection IP falls into any of them.
