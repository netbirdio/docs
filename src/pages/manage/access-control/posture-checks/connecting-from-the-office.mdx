...
export const description = "Use a Peer Network Range posture check so devices in the office reach the office network over their local link, and receive the NetBird route only when they are remote."
...
`netbird networks list` reports nothing available, and the system route table sends `192.168.1.0/24` over the local interface `en0`.
